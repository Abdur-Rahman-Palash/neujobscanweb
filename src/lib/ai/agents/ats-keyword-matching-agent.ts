...
export class ATSKeywordMatchingAgent {
  ...
  async matchKeywords(resumeData: ParsedResume, jobData: ParsedJob): Promise<AgentResult> {
    try {
      ...
      const categoryScores = this.calculateCategoryScores(resumeData, jobData);
  ...
  private performExactMatching(resumeKeywords: string[], jobKeywords: string[]): KeywordMatchResult['exactMatches'] {
    return jobKeywords.map(keyword => {
      ...
      const found = resumeKeywords.some(resumeKeyword => 
        this.normalizeKeyword(resumeKeyword) === normalizedKeyword
      );
        ...
        messages: [
          {
            ...
            Compare the resume text with job requirements and identify semantic matches.
  ...
  private calculateCategoryScores(resumeData: ParsedResume, jobData: ParsedJob): KeywordMatchResult['categoryScores'] {
    const resumeCategories = this.groupSkillsByCategory(resumeData.skills);
    const jobCategories = this.groupSkillsByCategory(jobData.skills);
  ...
  private groupSkillsByCategory(skills: any[]): { technical: string[], soft: string[], language: string[], tool: string[] } {
    const grouped = {
      technical: [] as string[],
      soft: [] as string[],
      language: [] as string[],
  ...
  async getKeywordSuggestions(resumeData: ParsedResume, jobData: ParsedJob): Promise<AgentResult> {
    try {
      ...
      if (missingKeywords.length === 0) {
        return {
          success: true,
          data: { suggestions: [], message: 'All keywords are covered!' },
        };
      }
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `You are an expert resume optimizer. Based on the job requirements and missing keywords, suggest how to incorporate these into the resume naturally.
            Return JSON with:
            {
              "suggestions": [
                {
