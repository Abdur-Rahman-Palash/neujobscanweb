import { ResumeParsingAgent } from './agents/resume-parsing-agent';
import { JobParsingAgent } from './agents/job-parsing-agent';
import { ATSKeywordMatchingAgent } from './agents/ats-keyword-matching-agent';
import { ScoringAgent } from './agents/scoring-agent';
import { SkillGapReasoningAgent } from './agents/skill-gap-reasoning-agent';
...
export class AIAgentOrchestrator {
  ...
  async processResume(resumeText: string, fileName: string): Promise<AgentResult> {
    try {
      ...
      const resumeParsingAgent = new ResumeParsingAgent(this.config.openaiApiKey);
  ...
  async processJobDescription(jobText: string): Promise<AgentResult> {
    try {
      ...
      const jobParsingAgent = new JobParsingAgent(this.config.openaiApiKey);
  ...
  async performATSScan(resumeData: any, jobData: any): Promise<AgentResult> {
    try {
      const scanId = this.generateScanId();
      
      // Step 1: ATS Keyword Matching
      const keywordAgent = new ATSKeywordMatchingAgent(this.config.openaiApiKey);
      const keywordResult = await keywordAgent.matchKeywords(resumeData, jobData);
      ...
      const scoringAgent = new ScoringAgent(this.config.openaiApiKey);
      const scoringResult = await scoringAgent.calculateATSScore(
        resumeData, 
        jobData, 
        keywordResult.data
      );
      ...
      const skillGapAgent = new SkillGapReasoningAgent(this.config.openaiApiKey);
      const skillGapResult = await skillGapAgent.analyzeSkillGaps(
        resumeData,
        jobData,
        scoringResult.data
      );
      ...
      const rewriteAgent = new RewriteAgent(this.config.openaiApiKey);
      ...
      const explanationAgent = new ExplanationAgent(this.config.openaiApiKey);
  ...
  async saveScanResult(userId: string, scanResult: any): Promise<AgentResult> {
    try {
      ...
      return {
        success: true,
        data: { saved: true, scanId: scanResult.scanId },
      };
    } catch (error) {
      return {
        success: false,
  ...
  async checkAgentHealth(): Promise<AgentResult> {
    try {
      const agents = [
        'ResumeParsingAgent',
        'JobParsingAgent', 
        'ATSKeywordMatchingAgent',
        'ScoringAgent',
        'SkillGapReasoningAgent',
        'RewriteAgent',
        'ExplanationAgent',
      ];
